...
public class UsersController extends HttpServlet {
	...
	private void insertUser(HttpServletRequest req, HttpServletResponse resp) {
		...
		if (userPW == null || userPW.trim().isEmpty()) {
			ControllerUtil.errorMessage(req, "Senha obrigatória para criar o usuário.");
			return;
		}
		...
		user.setPassword(userPW); // salva a senha criptografada
		...
		try {
			if (dao.save(user)) {
				...
				ControllerUtil.errorMessage(req, "Usuário '" + user.getName() + "' não pôde ser salvo.");
			}
		} catch (ModelException e) {
			e.printStackTrace();
			ControllerUtil.errorMessage(req, e.getMessage());
	...
	private void updateUser(HttpServletRequest req, HttpServletResponse resp) {
		...
		try {
			if (dao.update(user)) {
				...
				ControllerUtil.errorMessage(req, "Usuário '" + user.getName() + "' não pôde ser atualizado.");
			}
		} catch (ModelException e) {
			e.printStackTrace();
			ControllerUtil.errorMessage(req, e.getMessage());
